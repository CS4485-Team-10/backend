...
def process_claims_table(batch_size: int = 50):
    ...
    resp = (
        client.table("claims")
        .select(
            "claim_id, claim_text, sentiment_label, sentiment_score, fact_check_status"
        )
        .or_(
            "sentiment_label.is.null,sentiment_score.is.null,fact_check_status.is.null"
        )
        .limit(batch_size)
        .execute()
    )
    ...
    for i, row in enumerate(rows, 1):
        ...
        misinfo_label = (
            "misinfo"
            if entailment_label == "refuted" and entailment_conf >= 0.6
            else "clean"
            if entailment_label == "supported" and entailment_conf >= 0.6
            else "uncertain"
        )
        ...
        if (
            updates.get("fact_check_status") == "unverified"
            and misinfo_label == "misinfo"
        ):
